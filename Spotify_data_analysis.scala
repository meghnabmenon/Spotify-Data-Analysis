...
println("************************************************************** COMPARISON THE THREE DECADES - 80'S , 90'S , 2K ******************************************************")
...
//b) Filtering the songs released between 2000 - Present
val twok = yearly.filter(h => (h._8 >= "2000" && h._8 < "2010")) 
println("******************************** 2k DECADE  ********************************")
...
only showing top 10 rows
*/
println("********************************  Number of songs released in 2k  :  " + twok.count )
/*
********************************  Number of songs released in 2k  :  209984
*/
...
println("------------------------------------------------ 2k ------------------------------------------------------------")
//Finding POPULAR SONGS in 2k

// Finding the songs with popularity greater than 70 
val pop_songs_2k = twok.filter{h => h._3 >= 70}
pop_songs_2k.count // 36200

// Finding the popular artists who gave the popular songs
val pop_songs_per_arts_2k = pop_songs_2k.map(h => (h._6,1)).reduceByKey(_+_)
pop_songs_per_arts_2k.count //3010
...
// Percentage of Popular songs in 90 decade
val perc_popartists_2k = pop_songs_2k.count/twok.count.toDouble *100
println("********************************  % of popular songs in 2k  :  " + perc_popartists_2ks )
/*
********************************  % of popular songs in 2k  :  17.23940871685462
*/

// Finding POPULAR AND UNPOPULAR ARTISTS

// Finding POPULAR Artists
val unique_artists_2k = twok.map(h => (h._6,1)).reduceByKey(_+_)   // List of unique Artists from 2000's - key value pair
...
/*
******************************** NO. of artists with more than 300 songs: 54
*/
...
val data_freq2k = twok.filter{h => Setsortfreq_2k.contains(h._6)}   // Filtering the data of Artists with the most number of songs in 2k
val pop_songs_2k = data_freq2k.filter{h => h._3 >= 70} //The number of songs which are popular and belong to an artist with no.of songs>300 

val total_pop_songs_2k = mostsongs_artists.map(_._2).reduce(_+_) // The total number of songs of the artists with no.of songs>300 is = 16400
val Frac_pop_2k = pop_songs_2k.count/total_pop_songs_2k.toDouble *100
println("********************************  Fraction of popular songs from the popular artists : " + Frac_pop_2k) 
/*
********************************  Fraction of popular songs from the popular artists : 22.00609756097561
*/ 
...
/*
******************************** NO. of artists with less than 100 songs: 17953
*/ 


//val sortby_popartists_2k = data_pop2k.sortBy(_._6)                 
val freq2_key_least = leastsongs_artists.map(h => (h._1))                // Collecting just the Artists with the most number of songs in 2k (i.e. Key from KVpair)
val Setsortfreq_least_2k = freq2_key_least.collect.toSet                          // Creating a set of Artists with the most number of songs in 2k 
val data_freq_least2k = twok.filter{h => Setsortfreq_least_2k.contains(h._6)} 
...
val total_least_songs_2k = leastsongs_artists.map(_._2).reduce(_+_) // The total number of songs of the artists with no.of songs<100 is  57498
...
/*
********************************  Fraction of popular songs from the unpopular artists : 0.5234964694424153
*/  
...
val high_val_2k = twok.filter(h => h._18 > 0.8).map(h => (h._3,h._18)) // Filtering the high valence songs and Creating KVpair of (Popularity, Valence)
println("********************************  Songs with high valence in 2k  : " + high_val_2k.count )
/*
********************************  Songs with high valence corresponding  : 87649
*/
val high_val_pop_2k = high_val_2k.filter(_._1 > 70)
println("********************************  Popular high valence songs in 2k  : " + high_val_pop_2k.count)
/*
********************************  Popular high valence songs in 2k  : 10870
*/
...
/*
********************************  Fraction of popular high valence songs  : 12.401738753437003
*/

val low_val_2k = twok.filter(h => h._18 < 0.2).map(h => (h._3,h._18)) // Filtering the high valence songs and Creating KVpair of (Popularity, Valence)
println("********************************  Songs with low valence in 2k : " + low_val_2k.count )
/*
********************************  Songs with low valence in 2k : 85165
*/
val low_val_pop_2k = low_val_2k.filter(_._1 > 70)
println("********************************  Popular low  valence songs in 2k  : " +low_val_pop_2k.count)
/*
********************************  Popular low  valence songs in 2k  : 10475
*/
...
/*
********************************  Fraction of popular low valence songs  : 12.299653613573652*/
/*
...
val instr_high_2k = twok.filter(h =>h._16 > 0.8).map(h => (h._3,h._16))// Filtering the high instrumental songs and Creating KVpair of (Popularity, Instrumentalness)
instr_high_2k.count // 70338 - no of instrumental songs in 2k
val instr_high_pop_2k = instr_high_2k.filter(_._1 > 70)
instr_high_pop_2k.count // 1194 - no of popular Instrumental songs in 2k
...
/*
********************************  Fraction of popular intrumental songs in 2k  : 1.697517700247377
*/

val voc_2k = twok.filter(h =>h._16 < 0.7).map(h => (h._3,h._16))// Filtering the vocal songs and Creating KVpair of (Popularity, Instrumentalness)
...
/*
********************************  Fraction of popular vocal songs in 2k : 18.260971854762484
*/

/*
CONCLUSION: It is very evident that audience of 2k like vocal song more than instrumental songs 
*/
...
val studio_80s = eighties.filter(h =>h._16 < 0.7).map(h => (h._3,h._16))// Filtering the recorded songs and Creating KVpair of (Popularity, studio recorded)
...
val studio_90s = nineties.filter(h =>h._16 < 0.7).map(h => (h._3,h._16))// Filtering the recorded songs and Creating KVpair of (Popularity, studio recorded)
...
val live_high_2k = twok.filter(h =>h._17 > 0.8).map(h => (h._3,h._17))// Filtering the live performed songs and Creating KVpair of (Popularity, Liveness)
live_high_2k.count // 19198 - No of live recorded songs in 2k
val live_high_pop_2k = live_high_2k.filter(_._1 > 70)
live_high_pop_2k.count // 282 - No of popular live recorded songs in 2k
...
/*
********************************  Fraction of popular live recorded songs in 2k : 1.4689030107302845
*/
val studio_2k = twok.filter(h =>h._16 < 0.7).map(h => (h._3,h._16))// Filtering the recorded songs and Creating KVpair of (Popularity, studio recorded)
studio_2k.count  // 484558 - No of studio recorded songs 
val studio_pop_2k = studio_2k.filter(_._1 > 70)
studio_pop_2k.count //88485 - No of popular studio recorded songs in 2k
...
/*
********************************  Fraction of popular studio recorded songs in 2k:  18.260971854762484
*/
/*
CONCLUSION: Spotify listeners like to listen to studio recorded songs more than live performances which are recorded in 2k 
*/
...
val high_dance_eng_2k = twok.filter(h => h._9 > 0.8).map(h=> (h._9,h._10)) // filtering high danceable songs and creating KVpair - (danceablitly,energy)
val perc_dance_songs_2k = high_dance_eng_2k.count/twok.count .toDouble*100
println("******************************** Percentage of danceable songs in 2k : "+ perc_dance_songs_2k)
/*
******************************** Percentage of danceable songs in 2k : 9.037697281268917
*/
val high_eng_in_dance_2k = high_dance_eng_2k.filter(_._2 > 0.6)
val perc_high_eng_in_dance_2k = high_eng_in_dance_2k.count/high_dance_eng_2k.count.toDouble *100 //62.87
println("******************************** Percentage of danceable songs which also has high energy in 2k : "+ perc_high_eng_in_dance_2k)
/*
******************************** Percentage of danceable songs which also has high energy in 2k : 63.00646488963094
*/
/*
...
val after_2k = yearly.filter(h => (h._8 >= "2000" && h._8 < "2010")).filter(h => (h._14 >0.8)).count // 1788
...
val n = twok.count
val sum_eng = twok.map(_._10).reduce(_+_)
val sum_loud = twok.map(_._12).reduce(_+_)
val sum_prod_enlo = twok.map(h=> (h._10.*(h._12))).reduce(_+_)
val sum_sqr_en = twok.map(h => (h._10.*(h._10))).reduce(_+_)
val sum_sqr_lo = twok.map(h => (h._12.*(h._12))).reduce(_+_)
...
/*
******************************** Correlation between Energy and Loudness : 0.11546449415068569
*/
...
val sum_temp = twok.map(_._19).reduce(_+_)
val sum_prod_entm = twok.map(h=> (h._10.*(h._19))).reduce(_+_)
val sum_sqr_tm = twok.map(h => (h._19.*(h._19))).reduce(_+_)
...
/*
******************************** Correlation between Energy and Tempo : 0.017731737966057454
*/
...
val sum_aco = twok.map(_._15).reduce(_+_)
val sum_prod_aclo = twok.map(h=> (h._15.*(h._12))).reduce(_+_)
val sum_sqr_ac = twok.map(h => (h._15.*(h._15))).reduce(_+_)

println("******************************** Correlation between Acousticness and Loudness : " + corr_r(n,sum_aco,sum_loud,sum_prod_aclo,sum_sqr_ac,sum_sqr_lo))
/*
******************************** Correlation between Acousticness and Loudness : -0.20952611059360984
*/
